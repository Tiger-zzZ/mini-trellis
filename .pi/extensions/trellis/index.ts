...
function buildStartupContext(root: string): string {
  ...
  lines.push(
    "Memory: journal is git-durable session notes (`/trellis-remember` or `python3 ./.trellis/scripts/add_session.py`). Cross-session dialogue is `trellis mem list|search|context|extract`.",
...
export default function trellisExtension(pi: {
  ...
  pi.on?.("session_start", (_event, ctx) => {
    ctx?.ui?.notify?.(
      "mini-trellis context is available. Use /trellis-remember to persist journal notes.",
      "info",
    );
  });
